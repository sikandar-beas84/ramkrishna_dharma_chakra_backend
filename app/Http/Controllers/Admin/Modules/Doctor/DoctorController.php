...
class DoctorController extends Controller
{
    ...
    public function bannerUpdate(Request $request)
    {
    	...
    	if ($request->image) {
           @unlink(storage_path('app/public/banner_min/'.$banner->image));
    ...
    public function aboutUpdate(Request $request)
    {
    	...
    	if ($request->image) {
           @unlink(storage_path('app/public/about_min/'.$banner->image));
           @unlink(storage_path('app/public/about/'.$banner->image));
    	   $image = $request->image;
           $filename = time() . '-' . rand(1000, 9999) . '.' . $image->getClientOriginalExtension();
           $resize_image = Image::make($image->getRealPath());
           $resize_image->resize(1000, 1000, function($constraint){
    ...
    public function providerUpload(Request $request)
    {
    	if ($request->image) {
           ...
           $resize_image->resize(1000, 1000, function($constraint){
            ...
            //real image
            $image->move(storage_path('app/public/provider'),$filename);	
            $ins['image'] = $filename;
            Provider::create($ins);
    	}
    ...
    public function commonUseUpdate(Request $request)
    {
        ...
        if ($request->image) {
           @unlink(storage_path('app/public/common_min/'.$banner->image));
           @unlink(storage_path('app/public/common/'.$banner->image));
           $image = $request->image;
           $filename = time() . '-' . rand(1000, 9999) . '.' . $image->getClientOriginalExtension();
           $resize_image = Image::make($image->getRealPath());
           $resize_image->resize(1000, 1000, function($constraint){
            ...
            //real image
            $image->move(storage_path('app/public/common'),$filename);    
            $upd['image'] = $filename;
        }
