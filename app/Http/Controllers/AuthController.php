...
class AuthController extends Controller
{
   ...
   public function login(Request $request)
   {
       ...
       if (! $token = auth()->attempt($credentials,['exp' => Carbon::now()->addMonths(12)->timestamp])) {
   ...
   protected function respondWithToken($token)
   {
       ...
       User::where('id',auth()->user()->id)->update(['latest_login'=>$date]);
   ...
   public function sendOtp(Request $request)
   {
    ...
    public function profile()
    {
        ...
        try {
          $response['success'] = true;    
          $response['user_details'] = User::where('id',auth()->user()->id)->with('location_name')->first();
          $response['services'] = Service::where('status','!=','D')->with('location_name')->where('user_id',auth()->user()->id)->orderBy('id','desc')->get();
          $response['reviews'] = RattingModel::where('to_id',auth()->user()->id)->with('getFromUserDetails','service_details')->get();
          ...
          $response['notification_count'] = Notification::orderBy('id','desc')->where('read_status','N')->where('status','A')->where('status','!=','D')->count();
          $response['locations'] = LocationModel::where('status','A')->get();
          if ($response['rating_star']==null) {
              $response['rating_star'] = 0;
          }else{
              $response['rating_star'] = RattingModel::where('to_id',auth()->user()->id)->avg('ratting');
          }
          $response['image_url'] = 'https://beas.in/conveno/storage/profile/';
          $response['cover_image_url'] = 'https://beas.in/conveno/storage/cover/';
          return $response;
    ...
    public function profileUpdate(Request $request)
    {
        ...
        $image->move(storage_path('app/public/profile'),$filename);
        $upd['image'] = $filename;
        ...
        $filename = time() . '-' . rand(1000, 9999) . '.' . $image->getClientOriginalExtension();
        $image->move(storage_path('app/public/cover'),$filename);
        $upd['cover_photo'] = $filename;    

        // $cover_photo = $request->cover_photo;  // your base64 encoded
        // $cover_photo = str_replace('data:image/jpeg;base64,', '', $cover_photo);
        // $cover_photo = str_replace(' ', '+', $cover_photo);
        // $coverImage = str_random(10).'.'.'png';
        // \File::put(storage_path(). '/cover' . $coverImage, base64_decode($cover_photo));
        // $upd['cover_photo'] = 'cover'.$coverImage;
        }
    ...
    public function notification()
    {
        ...
        try {
          $response['success'] = true;  
          $response['notification'] = NotificationToUser::whereHas('notification',function($query){
                $query->orderBy('id','desc')->where('status','A');
          })->where('user_id',auth()->user()->id)->where('read_status','N')->with('notification')->get();
          // update-all-notification-as-read
          $user_id = explode(',',auth()->user()->id);
          NotificationToUser::whereIn('user_id',$user_id)->update(['read_status'=>'Y']);
          return $response;
    ...
    public function availUpdate(Request $request)
    {
        ...
        User::where('id',auth()->user()->id)->update(['availability'=>$request->availability]);
        $response['success'] = true;
        $response['message'] = 'User availability changed successfully';
        return $response;
